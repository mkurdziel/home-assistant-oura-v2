...
class OuraDataUpdateCoordinator(DataUpdateCoordinator[OuraData]):
    ...
    async def _async_update_data(self) -> OuraData:
        ...
        results = await asyncio.gather(
            _fetch(self._client.personal_info(), "personal_info"),
            _fetch(self._client.ring_configuration(), "ring_configuration"),
            _fetch(self._client.rest_mode_period(start_date, end_date), "rest_mode_period"),
            _fetch(self._client.daily_readiness(start_date, end_date), "daily_readiness"),
            _fetch(self._client.daily_sleep(start_date, end_date), "daily_sleep"),
            _fetch(self._client.daily_activity(start_date, end_date), "daily_activity"),
